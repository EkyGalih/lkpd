...
@section('content')
<h3><a href="{{ route('kode-rekening-admin') }}"><i class="fas fa-barcode"></i> Sub Kode Rekening [<u>{{ $nama_rekening }}</u>]</a></h3>
<div class="row mt">
    <div class="col-lg-8">
                ...
                <table class="table table-hover table-bordered table-striper" id="data-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('sub-kode-admin.edit', $rekening->sub_kode_id) }}" class="btn btn-warning btn-xs" data-tooltip="tooltip" data-placement="top" title="Edit Kode Rekening">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <button type="button" class="btn btn-danger btn-xs" data-tooltip="tooltip" data-placement="top" title="Hapus Kode Rekening" onclick="deleteData('{{ route('sub-kode-admin.destroy', $rekening->sub_kode_id) }}')">
    ...
    <div class="col-lg-4">
        <div class="content-panel" style="padding: 10px;">
            <form action="{{ isset($rekeningDetail) ? route('sub-kode-admin.update', $rekeningDetail->sub_kode_id) : route('sub-kode-admin.store') }}" method="POST" onsubmit="return validateForm()">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <select name="jenis_rekening" class="form-control" id="jenis_rekening" onchange="getSubKode()" required="true">
                        <option>Pilih</option>
                        @if (isset($rekeningDetail))
                        <option value="realisasi" {{ $rekeningDetail->kodeRekening->jenis_rekening == 'realisasi' ? 'selected' : '' }}>Realisasi Anggaran</option>
                        <option value="saldo" {{ $rekeningDetail->kodeRekening->jenis_rekening == 'saldo' ? 'selected' : '' }}>Saldo Anggaran Lebih</option>
                        <option value="neraca" {{ $rekeningDetail->kodeRekening->jenis_rekening == 'neraca' ? 'selected' : '' }}>Neraca</option>
                        <option value="operasional" {{ $rekeningDetail->kodeRekening->jenis_rekening == 'operasional' ? 'selected' : '' }}>Operasional</option>
                        <option value="arus_kas" {{ $rekeningDetail->kodeRekening->jenis_rekening == 'arus_kas' ? 'selected' : '' }}>Arus Kas</option>
                        <option value="ekuitas" {{ $rekeningDetail->kodeRekening->jenis_rekening == 'ekuitas' ? 'selected' : '' }}>Perubahan Ekuitas</option>
                        @else
                ...
                @if (isset($rekeningDetail))
                <a href="{{ route('kode-rekening-admin.show', $rekeningDetail->kode_rekening_id) }}" class="btn btn-warning btn-sm"><i class="fas fa-sync"></i> Reset</a>
                @endif
            </form>
        </div>
    </div>
