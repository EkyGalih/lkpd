    ...
    <tbody>
        @foreach ($Pengguna as $user)
            <tr>
                ...
                <td>
                    <button type="button" class="btn btn-warning btn-xs" data-toggle="modal" data-target="#EditData{{ $loop->iteration }}" data-tooltip="tooltip" data-placement="top" title="Edit Pengguna">
                        <i class="fas fa-user-cog"></i>
                    </button>
                    @if ($user->jenis_pegawai != 'admin')
                    <button type="button" class="btn btn-danger btn-xs" onclick="deleteData('{{ route('admin-pengguna.destroy', $user->user_id) }}')" data-tooltip="tooltip" data-placement="top" title="Hapus Pengguna">
